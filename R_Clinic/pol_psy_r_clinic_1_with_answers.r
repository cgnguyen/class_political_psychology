#Setup-------------
  ...
  D<-read_dta(file.choose())
  
  D <- read_dta("D:/OneDrive/Teaching/class_political_psychology/R_Clinic/dezim_aar_wide_c_100.dta")
 ...
 ##Faktor Variable- Geschlecht---------------------
  ...
  D<-
    D%>%
      mutate(
        bundesland=as_factor(v_16_w1),
        age_group=as_factor(v_12_g_w1),
      )
  
  summary(D$bundesland)
 
  ##Aufgabe 1-----------------
  #Bereinigit die Variable  Hauptaktivität (F2, v_28_w1)
